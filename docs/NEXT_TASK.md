...
1. **既存`/search`画面の改善**（ユーザー指定、2026-07-19、共有機能の次に着手予定だったタスク）: ジャンル・タグフィルタが未実装のまま（[KNOWN_ISSUES.md](KNOWN_ISSUES.md)参照）。なろうの`genre`は数値コード、カクヨムの`genre`は英語コード（例: `FANTASY`）のまま保存されており、人間可読名への変換もセットで検討が必要。
2. **ユーザーによる実機確認**（[USER_TODO.md](USER_TODO.md)参照）: `https://novelshelf.jp`にご自身のiPhone/Androidでアクセスし、①②③の修正・カクヨム/ハーメルン対応・共有機能が実際に効いているか確認してもらう。
3. **（任意）パスワードリセットを実際に使うためのSMTP接続情報を`.env`に設定**（[USER_TODO.md](USER_TODO.md)）: 機能自体は実装済み・動作確認済み。プロバイダを決めて接続情報を設定するだけで使えるようになる。急ぎではない。
4. **（任意・iPhone/iPadのみ）共有機能用のiOSショートカット作成**（[USER_TODO.md](USER_TODO.md)）: Web Share Target API非対応のための代替手段。
