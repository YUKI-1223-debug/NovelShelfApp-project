...
**Phase1〜Phase6（初回デプロイ）完了。`https://novelshelf.jp`で本番稼働中**。ユーザー報告3件＋追加要望1件（本棚追加時のタイトル欠落・検索機能の作り直し・読書画面のお気に入り・統計画面の廃止）に対応したコミット(`8b91989`)をVPSへデプロイ済み（`docker compose ps`で4コンテナhealthy、`https://novelshelf.jp/`が200を確認済み、2026-07-26）。同日、追加で「追加順」ソート不具合の修正と本棚検索へのサイト別フィルター追加を実施（下記「追加順ソート修正・サイト別フィルター追加」参照）、デプロイはこれから。
